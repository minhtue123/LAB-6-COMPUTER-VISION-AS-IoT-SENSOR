...
METADATA_FIELDS = [
    ...
    "processed_path", "width", "height", "brightness",
    "processing_status", "processing_time_ms", "note"
]

EVENT_FIELDS = [
    "event_id", "image_id", "timestamp", "event_type",
    "score", "severity", "explanation", "action_hint"
]
...
def save_image_unicode(path: Path, image_bgr: np.ndarray) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
...
def create_processed_contact_sheet(frame_bgr: np.ndarray, image_id: str) -> Tuple[Path, float, Dict[str, Any]]:
    ...
    save_image_unicode(out_path, sheet)
...
def log_image_pipeline(frame_bgr: np.ndarray, source_type: str, device_id: str, note: str = "") -> Dict[str, Any]:
    ...
    save_image_unicode(raw_path, frame_bgr)
...
def simulated_frame(counter: int = 0, width: int = 640, height: int = 360) -> np.ndarray:
    ...
    cv2.putText(frame, "SIMULATED CAMERA STREAM", (25, 35),
                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2)
    cv2.putText(frame, "Use source=0 for laptop camera or an IP camera URL",
                (25, height - 25), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 0, 0), 1)
...
def stream_frames(source: str = "0") -> Iterable[bytes]:
    ...
    while True:
        ...
        cv2.putText(frame, f"{source_label} | source={source} | frame={counter}",
                    (10, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.58, (0, 0, 0), 2)
...
def motion_capture(source: str, seconds: int = 8, threshold: int = 25, min_area: int = 800) -> Dict[str, Any]:
    ...
    result = log_image_pipeline(
        ...
        note=f"motion_score={round(best_score, 2)}, threshold={threshold}, min_area={min_area}"
    )
...
app = FastAPI(
    title="Lab 6 - Computer Vision as IoT Sensor",
    description="Camera stream, snapshot, video, motion, metadata, image event and dashboard."
)
...
def latest() -> Dict[str, Any]:
    ...
    if latest_meta:
        ...
        raw_url = relative_url(ROOT / image_path)
        processed_url = relative_url(ROOT / processed_path)
